...
# split(): 구분자로 분리
str1 = "1,홍길동,100,100,90"
s = str1.split(",")
print(s)
print(s[4]) # 문자열임

# 문자열을 리스트로 변환해서 번호, 이름, 국어, 영어, 수학, 합계, 평균을 출력하시오.
# 리스트에 합계, 평균만 추가되면 됨
str1 = "1,홍길동,100,100,99"

s = str1.split(",") # [1,홍길동,100,100,99]로 변환

s[2] = int(s[2])
s[3] = int(s[3])
s[4] = int(s[4])
s.append(s[2]+s[3]+s[4]) # 합계 추가
s.append(s[5]/3) # 평균 추가

print("[학생성적프로그램]")
print("번호\t이름\t국어\t영어\t수학\t합계\t평균")
print("-"*60)
print("{}\t{}\t{}\t{}\t{}\t{}\t{:.2f}".format(*s)) # s에 들어 있는 요소를 분해해서 입력해줌
print(f"{s[0]}\t{s[1]}\t{s[2]}\t{s[3]}\t{s[4]}\t{s[5]}\t{s[6]:.2f}") # s에 들어 있는 요소를 분해해서 입력해줌

# *s: 구조분해할당(s[0],s[1],s[2],s[3],s[4],s[5],s[6])

arr = [1,2,3,4,5]
print(arr)
print(*arr) # a[0],a[1],a[2],a[3],a[4]으로 입력한 것과 동일함
